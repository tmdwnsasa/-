...
#pragma region UI_Test
	for (int32 i = 0; i < 6; i++)
	{
		...
		obj->GetTransform()->SetLocalPosition(Vec3(-350.f + (i * 110), 150.f, 500.f));
		shared_ptr<MeshRenderer> meshRenderer = make_shared<MeshRenderer>();
		{
...
#pragma region Menu
	{
		shared_ptr<GameObject> Menu = make_shared<GameObject>();
		Menu->SetName(L"Menu");
		Menu->AddComponent(make_shared<Transform>());
		Menu->AddComponent(make_shared<MainMenu>());
		Menu->GetTransform()->SetLocalPosition(Vec3(0, 1000, 500));
		scene->AddGameObject(Menu);
	}
#pragma endregion
