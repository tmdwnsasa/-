...
shared_ptr<Mesh> Mesh::CreateFromFBX(const FbxMeshInfo* meshInfo, vector<shared_ptr<FbxAnimClipInfo>>& animClip, vector<shared_ptr<FbxBoneInfo>>& bone)
{
	shared_ptr<Mesh> mesh = make_shared<Mesh>();
	mesh->CreateVertexBuffer(meshInfo->vertices);
	...
	if (meshInfo->hasAnimation)
		mesh->CreateBonesAndAnimations(animClip, bone);
...
void Mesh::CreateBonesAndAnimations(vector<shared_ptr<FbxAnimClipInfo>>& animClip, vector<shared_ptr<FbxBoneInfo>>& bone)
{
#pragma region AnimClip
	uint32 frameCount = 0;
	vector<shared_ptr<FbxAnimClipInfo>>& animClips = animClip;
	for (shared_ptr<FbxAnimClipInfo>& ac : animClips)
	{
		AnimClipInfo info = {};
...
#pragma region Bones
	vector<shared_ptr<FbxBoneInfo>>& bones = bone;
	for (shared_ptr<FbxBoneInfo>& bone : bones)
	{
		BoneInfo boneInfo = {};
		boneInfo.parentIdx = bone->parentIndex;
